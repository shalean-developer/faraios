...
async function runPleskDomainPostConnect(input: {
  ...
  const readiness = await getDomainHostingReadiness(input.companyId, input.domain);
  if (!readiness.ready) {
    console.warn(
      "[hosting-domain] Skipping Plesk post-connect — hosting not ready",
      input.domain
    );
    return;
  }

  const serverId = input.serverId ?? readiness.serverId;
  const pleskSubscriptionId = input.pleskSubscriptionId ?? readiness.pleskSubscriptionId;

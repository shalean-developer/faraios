...
export default async function CompanyWebsitesPage({ params }: Props) {
  ...
  const memberSlug = Array.isArray(typedMembership?.companies)
    ? typedMembership?.companies?.[0]?.slug ?? null
    : typedMembership?.companies?.slug ?? null;

  if (!typedMembership?.company_id || memberSlug !== slug) {
  ...
  const { data: websites } = await supabase
    .from("websites")
    .select("*")
    .eq("client_id", typedMembership.company_id)
    ...
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Create and manage websites for your workspace.
          </p>
        </div>
        <Link

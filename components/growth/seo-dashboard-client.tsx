...
export function SeoDashboardClient({
  ...
  function saveSettings() {
    ...
    startTransition(async () => {
      ...
      else setMessage("Local SEO settings saved.");
    });
  }
  ...
  function generatePages() {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) setError(result.error);
      else setMessage(`Generated ${result.created ?? 0} draft service area page(s).`);
    });
  }
  ...
  function publishPage(pageId: string) {
    startTransition(async () => {
      ...
      if (!result.ok) setError(result.error);
      else setMessage("Page published.");
    });
  }
  ...
  function saveWebsiteSeo(website: WebsiteSeo, seoTitle: string, seoDescription: string) {
    startTransition(async () => {
      ...
      if (!result.ok) setError(result.error);
      else setMessage(`SEO saved for ${website.name}.`);
    });
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard label="SEO score" value={`${audit.score}/100`} />
        ...
        <MetricCard label="Sitemap" value={audit.sitemapStatus} />
        <MetricCard label="Schema" value={audit.schemaStatus} />
        <MetricCard label="Indexed pages (est.)" value={String(audit.indexedPagesPlaceholder)} />
      </section>

...
import type { QuoteLineItem } from "@/types/financial";
...
function rowsFromLineItems(items: QuoteLineItem[]): LineItemRow[] {
  return items.map((li) => ({
    id: li.id,
...
export function QuoteEditPanel({
  ...
  onSaved: () => void;
  onCancel: () => void;
  saveAction: typeof import("@/app/actions/quotes").updateQuoteAction;
}) {
  const [rows, setRows] = useState<LineItemRow[]>(() => rowsFromLineItems(lineItems));
  const [notesValue, setNotesValue] = useState(notes ?? "");
  const [validUntilValue, setValidUntilValue] = useState(validUntil?.slice(0, 10) ?? "");
  ...
  return (
    ...
    >
      <h2 className="text-base font-semibold text-slate-900">Edit quote</h2>
      <p className="mt-1 text-sm text-slate-500">
        Update line items, notes, and validity. Changes are logged in the audit trail.
      </p>
